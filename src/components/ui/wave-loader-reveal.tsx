...
export function WaveLoaderReveal({ onComplete }: WaveLoaderRevealProps) {
  ...
  useEffect(() => {
    ...
    const preCurtain = document.getElementById("pre-react-curtain")
    if (preCurtain) {
      preCurtain.style.opacity = "0"
      setTimeout(() => preCurtain.remove(), 400)
    }
    ...
    const render = (now: number) => {
      ...
      if (elapsed < holdDuration) {
        ...
        if (badgeRef.current) {
          ...
          badgeRef.current.style.transform = `translate(-50%, -50%) scale(${pulse.toFixed(3)})`
        }
      ...
      const badgeAlpha = Math.max(0, 1 - rawProgress / 0.28)
      ...
      if (badgeRef.current) {
        ...
        badgeRef.current.style.transform = `translate(-50%, -50%) scale(${badgeScale.toFixed(3)})`
      }
  ...
  return (
    ...
    >
      ...
      <div
        ref={badgeRef}
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none flex items-center justify-center w-[76px] h-[76px] sm:w-[84px] sm:h-[84px] rounded-full bg-white/75 backdrop-blur-2xl border-[1.5px] border-[#70CAB9]/45 shadow-[0_20px_54px_-6px_rgba(15,127,117,0.25),0_2px_10px_rgba(0,0,0,0.03),inset_0_1px_2px_rgba(255,255,255,1)] will-change-[transform,opacity]"
      >
        <svg
